...
export const CreateResultSchema = z.object({
    test1: z.number({ message: 'Test1 must be a number' }).max(10).optional().nullable(),
    test2: z.number({ message: 'Test2 must be a number' }).max(10).optional().nullable(),
    mid: z.number({ message: 'Mid must be a number' }).max(20).optional().nullable(),
    final: z.number({ message: 'Final must be a number' }).max(40).optional().nullable(),
    // conduct: z.nativeEnum(Conduct).optional(), // Updated to use the Conduct enum
    assignment: z.number({ message: 'Assignment must be a number' }).max(15).optional().nullable(),
    quiz: z.number({ message: 'Quiz must be a number' }).max(5).optional().nullable(),
